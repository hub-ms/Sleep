        ...
        when (intent.action) {
            ACTION_START -> {
                ...
                if (!trackingManager.trackingState.value.isTracking) {
                    ...
                    trackingManager.performStart(sessionId, durationMillis, musicTitle)
                }
            }
            ACTION_FINISH -> {
                ...
                val musicTitle = intent.getStringExtra(EXTRA_MUSIC_TITLE) ?: currentState.musicTitle
                trackingManager.performFinish(sessionId, musicTitle)
            }
            ACTION_DISCARD -> {
                val currentState = trackingManager.trackingState.value
                val sessionId = intent.getStringExtra(EXTRA_SESSION_ID) ?: currentState.sessionId ?: return START_STICKY
                val musicTitle = intent.getStringExtra(EXTRA_MUSIC_TITLE) ?: currentState.musicTitle
                trackingManager.performDiscard(sessionId, musicTitle)
            }
        }
        return START_STICKY
    }
